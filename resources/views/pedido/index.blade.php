    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <!-- Botón para agregar nuevo pedido -->
                <div class="flex justify-end p-2 mr-4"> 
                            <!--Url de la ruta para pedido.create-->                   
                ...
                <table id="producto" class="w-full text-base border text-center text-gray-500 dark:text-gray-700">
                    ...
                    <tbody>
                        @foreach ($pedidos as $items)
                            <tr class="bg-white border-b dark:border-gray-700 border-gray-200 hover:bg-gray-100 dark:hover:bg-gray-300">
                                <td class="p-2">{{$items->ID_PEDIDO}}</td>
                                <td>{{$items->Fecha_de_compra}}</td>
                                <td>{{$items->Fecha_de_entrega}}</td>
                                <td>{{$items->Total_de_pago}}</td>
                                <td>{{$items->Estado_pedido}}</td>
                                <td class="px-6 py-4 gap-2 flex justify-center">
                                    <a href="{{ route('pedido.edit', $items) }}"
                                        class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-md">
                                        Editar
                                    </a>
                                    <form action="{{ route('pedido.destroy', $items) }}" method="POST"
